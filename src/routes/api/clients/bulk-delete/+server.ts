...
import { deleteClients } from '$lib/server/services';
import { bulkDeleteClientsSchema } from '$lib/validations';
...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	const parsed = bulkDeleteClientsSchema.safeParse(body);
	...
	try {
		const deletedIds = await deleteClients(locals.user.id, parsed.data.ids);
